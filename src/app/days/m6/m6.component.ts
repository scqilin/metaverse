...
export class M6Component implements OnInit {
  ...
  initScene() {
    ...
    const geometrySphere = new THREE.CircleGeometry(spheredr+3, 64);
    const materialSphere = new THREE.MeshPhongMaterial({ color: 0x588050, side: THREE.DoubleSide });
    ...
    let addOneTree = (x: number, y: number, z: number) => {
      ...
      groupall.add(group);
      const geometry = new THREE.BoxGeometry(0.2 * scaleK,1 * scaleK, 0.2 * scaleK );
      const cube = new THREE.Mesh(geometry, materialTrunk);
      cube.position.set(0,  scaleK * 0.5,0);
      ...
      cone.position.set(0, 1.5 * scaleK,0);
      cone.castShadow = true;
      group.add(cone);
      this.tweenScale(group, 0.1, 1+Math.random(), 2000)
    ...
    setInterval(() => {
      _i += 1;
      const s = (1+.3*Math.sin(_i*11227)+Math.sin(_i*3/103) + Math.sin(_i/171) )* Math.PI * simplex.noise(cube.position.x*simk,  cube.position.z*simk, );
      uv.u += 1*Math.sin(s);
      uv.v += 1*Math.cos(s);

      let xyz = this.uvToxyz(uv.u, uv.v, spheredr );

      cube.position.set(xyz[0],0, xyz[2]);
      for (let i = grps.length - 1; i > 0; i--) {
        grps[i].position.copy(grps[i - 1].position);
      }
      grps[0].position.set(xyz[0], 0, xyz[2]);
      //相机跟随
      camera.lookAt(xyz[0], 2, xyz[2]);
      camera.position.set(grps[grps.length - 1].position.x,3,grps[grps.length - 1].position.z);
      treearr.forEach((group:any) => {
        ...
        if (distance < 1 && group.visible === true) {
          ...
          group.position.set(xyzr[0], 0, xyzr[2]);
          this.tweenScale(group, 0.1, 1+Math.random(), 2000)
